...
DIAG_DIRNAME  = "Ollama ボトルネック診断ツール v2 by くろぴな"
DIAG_FILENAME = "ollama_diagnosis_gui.py"

def find_diagnosis_tool():
    """Ollama診断GUIの.pyを候補順に探して、最初に見つかった絶対パスを返す（無ければ""）。"""
    here = os.path.dirname(os.path.abspath(__file__))
    candidates = []
    # ① 環境変数で明示指定（フルパス or フォルダ）＝確実な上書き手段
    env = os.environ.get("VRAMAPYON_DIAG", "").strip().strip('"')
    if env:
        candidates.append(env if env.lower().endswith(".py")
                          else os.path.join(env, DIAG_FILENAME))
    # ② 同梱（リポジトリに同じ階層 or サブフォルダで入れた場合）
    candidates.append(os.path.join(here, DIAG_DIRNAME, DIAG_FILENAME))
    candidates.append(os.path.join(here, "diag", DIAG_FILENAME))
    # ③ 隣（本拠地で診断ツールを兄弟フォルダとして公開した場合）
    candidates.append(os.path.join(here, "..", DIAG_DIRNAME, DIAG_FILENAME))
    # ④ 2つ上（旧レイアウト＝工房でアプリの2階層上に診断ツールがあった頃）
    candidates.append(os.path.join(here, "..", "..", DIAG_DIRNAME, DIAG_FILENAME))
    for c in candidates:
...
$ded = @{}   # "pid|luid" -> bytes
try { $samples = (Get-Counter '\GPU Process Memory(*)\Dedicated Usage').CounterSamples }
catch { $samples = @() }
foreach ($s in $samples) {
  if ($s.CookedValue -gt 0 -and
      $s.InstanceName -match 'pid_(\d+)_luid_(0x[0-9a-fA-F]+_0x[0-9a-fA-F]+)_phys') {
    $key = "$($matches[1])|$($matches[2])"
    if ($ded.ContainsKey($key)) { $ded[$key] += [int64]$s.CookedValue }
    else { $ded[$key] = [int64]$s.CookedValue }
  }
}
$pidTotal = @{}; $pidAdp = @{}
foreach ($k in $ded.Keys) {
  $parts = $k.Split('|'); $procId = [int]$parts[0]; $luid = $parts[1]; $b = $ded[$k]
  if ($pidTotal.ContainsKey($procId)) { $pidTotal[$procId] += $b } else { $pidTotal[$procId] = $b }
  if (-not $pidAdp.ContainsKey($procId)) { $pidAdp[$procId] = @{} }
  $pidAdp[$procId][$luid] = $b
}
$procs = foreach ($procId in $pidTotal.Keys) {
  ...
  $adps = foreach ($lu in $pidAdp[$procId].Keys) {
    [pscustomobject]@{ luid = $lu; bytes = $pidAdp[$procId][$lu] }
  }
  [pscustomobject]@{ pid = $procId; bytes = $pidTotal[$procId]; name = $nm;
...
def get_vram():
    ...
    if isinstance(procs, dict):
        procs = [procs]
    procs = [d for d in procs if d.get("bytes", 0) and d["bytes"] > 0]
...
class VRAMaPyon(ctk.CTk):
    ...
    def _open_diagnosis(self):
        ...
        if not path:
            # 候補のうち分かりやすい場所(同梱想定の隣)を案内に出す
            here = os.path.dirname(os.path.abspath(__file__))
            hint = os.path.normpath(os.path.join(here, DIAG_DIRNAME, DIAG_FILENAME))
    ...
    def _update_row(self, row, it):
        ...
        cell._sub = self._pid_text(it["pid"], it["gtag"])
